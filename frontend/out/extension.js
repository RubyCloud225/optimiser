...
function activate(context) {
    let disposable = vscode.commands.registerCommand('extension.optimizeCode', async () => {
        ...
        try {
            const response = await axios_1.default.post('http://localhost:8000/optimize', {
                code,
            ...
            if (!panel) {
                panel = vscode.window.createWebviewPanel('cyberpunkCodeOptimizer', 'Cyberpunk Code Optimizer', vscode.ViewColumn.Beside, {
                    enableScripts: true,
                    localResourceRoots: []
                });
                panel.onDidDispose(() => {
...
function deactivate() { }
function getWebviewContent(code, language) {
